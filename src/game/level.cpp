...
void Level::load(std::string filename, bool editor) {
...
void Level::update() {
    ...
    if (editor) {
        ...
        if (IsKeyPressed(KEY_B)) {
            for (int i = 100; i < levelSizeX - 100; i++) {
                Vec2 pos = {i * tileSize, (levelSizeY - j - 100) * tileSize};
                placeTile(pos, placedBlockId);
            }
            j++;
        }
    }
